  ...
  return (
    <div className="job-detail-container">
      ...
      <div className="icon-table-container">
        ...
        <div className="detail-steps-container">
          {steps.map((step, i) => {
            if (i != steps.length - 1)
              return (
                <div className="detail-step-container" key={i}>
                  <div className="detail-step">step {i + 1}</div>
                  <div className="step-message">{step.text}</div>
                </div>
              );
          })}
        </div>
      ...
      <div className="detail-status-container">
        ...
        <div className={`${status} status-container`}>{status}</div>
      </div>
    </div>
  );
};
