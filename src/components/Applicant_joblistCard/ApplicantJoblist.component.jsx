...
{isCollapsed ? 
  <AiFillDownCircle className=' hover:scale-125 cursor-pointer  ' onClick={Handleclick}/> : <IoIosArrowDropupCircle className='hover:scale-125 cursor-pointer' onClick={Handleclick}/>
...
}
 {
                !isCollapsed && (<>
                    ...
                    <div className='flex'>
                        {  
                        !opened && (  <label className='relative right-44   text-sm mr-4'>Choose a file:
        <input type="file"  className='border  border-black bg-cust-secondary text-cust-primary  rounded-sm' onChange={HandleFileInputChange} />
      </label>
      )
      }
        </div>
